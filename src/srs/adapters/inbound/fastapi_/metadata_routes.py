...
"""FastAPI routes for Experimental Metadata operations."""
...
async def upsert_metadata(
    ...
    try:
        await registry.metadata.upsert_metadata(
            study_id=study_id, metadata=body.metadata
        )
    except MetadataPort.StudyNotFoundError as err:
        raise HttpStudyNotFoundError(study_id=study_id) from err
    except MetadataPort.StatusConflictError as err:
...
async def get_metadata(
    ...
    try:
        return await registry.metadata.get_metadata(study_id=study_id)
    except MetadataPort.MetadataNotFoundError as err:
...
async def delete_metadata(
    ...
    try:
        await registry.metadata.delete_metadata(study_id=study_id)
    except MetadataPort.StudyNotFoundError as err:
        raise HttpStudyNotFoundError(study_id=study_id) from err
    except MetadataPort.StatusConflictError as err:
        raise HttpStatusConflictError(detail=str(err)) from err
